...
public class CmdSetrank extends PowerCommand {

    private Users users;

    public CmdSetrank(PowerRanks plugin, String command_name, COMMAND_EXECUTOR ce) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String commandName, String[] args) {
        if (args.length == 2) {
            ...
            if (sender instanceof Player) {
                for (PRPermission permission : PowerRanks.getInstance().getEffectivePlayerPermissions((Player) sender)) {
                    if (permission.getName().equalsIgnoreCase("powerranks.cmd." + commandName.toLowerCase() + "." + target_rank)) {
