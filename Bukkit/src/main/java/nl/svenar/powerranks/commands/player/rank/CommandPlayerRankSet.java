...
public class CommandPlayerRankSet extends PowerCommand {

    public CommandPlayerRankSet(PowerRanks plugin, COMMAND_EXECUTOR ce, boolean showInHelp) {
    ...
    public String getDescription() {
        return "Override all player's ranks and set it to the specified rank";
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if (!sender.hasPermission("powerranks.command.player.rank.set")) {
        ...
        if (args.length == 2) {
            ...
            switch (PowerRanks.getAPI().setPlayerRank(playerName, rankName)) {
                ...
                case PLAYER_SUCCESSFULLY_SET_RANK:
                    sender.sendMessage(plugin.pluginChatPrefix() + ChatColor.GREEN + plugin.getLangConfig()
                            .getNode("plugin.commands.player.general.player-rank-set-successfully"));
                    break;
