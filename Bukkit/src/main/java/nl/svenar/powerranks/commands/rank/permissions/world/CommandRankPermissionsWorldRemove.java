...
public class CommandRankPermissionsWorldRemove extends PowerCommand {

    public CommandRankPermissionsWorldRemove(PowerRanks plugin, COMMAND_EXECUTOR ce, boolean showInHelp) {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        ...
        if (args.length == 3 || args.length == 4) {
            ...
            switch (PowerRanks.getAPI().removeWorldFromRankPermission(rankName, permissionName, worldName)) {
                ...
                case RANK_PERMISSION_FAILED_WORLD_DOES_NOT_EXIST:
                    sender.sendMessage(plugin.pluginChatPrefix() + ChatColor.GREEN + plugin.getLangConfig()
                            .getNode("plugin.commands.rank.permissions.add.world.does-not-exist"));
                    break;

                case RANK_PERMISSION_WORLD_REMOVED_SUCCESSFULLY:
                    sender.sendMessage(plugin.pluginChatPrefix() + ChatColor.GREEN + plugin.getLangConfig()
                            .getNode("plugin.commands.rank.permissions.add.world.removed-successfully"));
                    break;
                default:
                    break;
            }
    ...
    public ArrayList<String> tabCompleteEvent(CommandSender sender, String[] args) {
        ...
        if (args.length == 3) {
            PRRank rank = BaseDataHandler.getRank(args[0]);
            if (rank != null) {
                PRPermission permission = rank.getPermission(args[1]);
                if (permission != null) {
